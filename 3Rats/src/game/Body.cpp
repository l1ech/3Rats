...
Body::Body() { 
    name = "NULL";
    index = current_index++; 
    std::cout << "[Body" << index << "]: CONSTRUCTOR BODY:  " << std::endl;
    std::cout << std::endl;
}

Body::~Body() { 
    SDL_DestroyTexture(texture); 
    std::cout << "[Body" << index << "]: DESTRUCTOR BODY: " << index << std::endl;
    std::cout << std::endl;
}
...
void Body::draw(SDL_Renderer* ptr_renderer) const{
    std::cout << "[Body" << index << "]: " << name << "draw called" << std::endl;

    // Check if renderer is valid
    if (ptr_renderer == nullptr) {
        std::cerr << "[Body" << index << "]: Error with <" << name << "> Renderer is null. Cannot draw." << std::endl;
        return;
    }

    // Check if texture is valid
    if (texture == nullptr) {
        std::cerr << "[Body" << index << "]: Error with <" << name << "> Texture is null. Cannot draw." << std::endl;
        return;
    }

    // Check crop rectangle dimensions
    if (crop_rect.w <= 0 || crop_rect.h <= 0) {
        std::cerr << "[Body" << index << "]: Error with <" << name << "> Invalid crop rectangle dimensions: width=" << crop_rect.w << ", height=" << crop_rect.h << ". Cannot draw." << std::endl;
        return;
    }

    // Check position rectangle dimensions
    if (position_rect.w <= 0 || position_rect.h <= 0) {
        std::cerr << "[Body" << index << "]: Error with <" << name << "> Invalid position rectangle dimensions: width=" << position_rect.w << ", height=" << position_rect.h << ". Cannot draw." << std::endl;
        return;
    }

    // Attempt to draw
    if (SDL_RenderCopy(ptr_renderer, texture, &crop_rect, &position_rect) != 0) {
        std::cerr << "[Body" << index << "]: Error with <" << name << "> SDL_RenderCopy failed: " << SDL_GetError() << std::endl;
        //
    } else {
        std::cout << "[Body" << index << "]: " << name << " drawn successfully at position x: "
...
void Body::set_surface(SDL_Renderer* ptr_renderer) {
    std::cout << "[Body" << index << "]: " << name << " set_surface called" << std::endl;
...
void Body::set_texture(const std::string& filePath) {
    ...
    if (texture == nullptr) {
        std::cerr << "[Body]: Failed to load texture from " << filePath << std::endl;
    } else {
        std::cout << "[Body]: Texture loaded successfully from " << filePath << std::endl;
    }
}

void Body::set_cords(int x, int y)
{
    std::cout << "[Body" << index << "]: " << name << "set_cords called with x: " << x << ", y: " << y << std::endl;
    ...
    std::cout << "[Body" << index << "]: "<< name << "set_cords to x: " << x << ", y: " << y << std::endl;
}
...
SDL_Texture* Body::LoadTexture(const std::string& filePath, SDL_Renderer* ptr_renderer) {
    std::cout << "[Body" << index << "]: LoadTexture called for file: " << filePath << std::endl;
    SDL_Texture* texture = nullptr;
    SDL_Surface* surface = IMG_Load(filePath.c_str());
    if (surface == nullptr) {
        std::cerr << "[Body" << index << "]: Error loading surface: " << IMG_GetError() << std::endl;
    } else {
        texture = SDL_CreateTextureFromSurface(ptr_renderer, surface);
        if (texture == nullptr) {
            std::cerr << "[Body" << index << "]: Error creating texture: " << SDL_GetError() << std::endl;
        } else {
            std::cout << "[Body" << index << "]: Texture created successfully from surface" << std::endl;
        }
